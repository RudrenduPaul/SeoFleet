...
_SKILL_RELATIVE_PATH = os.path.join(".claude", "skills", "seofleet", "SKILL.md")
...
project's configured site using the `seofleet` CLI.
...
def init_project(target_path: str, site_url: str = "") -> InitResult:
    """
    Scaffolds a working SeoFleet setup into a target directory: a
    seofleet.json the `check`/`fleet` commands read, plus a minimal Claude
    Code skill file that points at this same CLI. Idempotent: an existing
    seofleet.json or SKILL.md is left untouched so re-running init never
    ...
    try:
        ...
        raise SeoFleetError(f'Could not create directory "{target_path}": {err}', 2) from err
