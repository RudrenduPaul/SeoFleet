"""
Thin argument-parsing wrapper over seofleet.cli_lib. Ported from src/cli.ts
...
`seofleet [options] <command>`, installed via the `seofleet` console-script
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-V", "--version", action="version", version=f"seofleet-cli {_VERSION}")
    parser.add_argument(
    ...
    init_parser = subparsers.add_parser(
        "init",
        help="Scaffold a SeoFleet setup (seofleet.json + a Claude Code skill file) into a target directory",
    )
