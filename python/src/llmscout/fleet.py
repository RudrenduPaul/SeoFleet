...
def load_fleet_manifest(manifest_file: str) -> FleetManifest:
    ...
    if not os.path.exists(manifest_file):
        raise LLMScoutError(f'Fleet manifest not found: "{manifest_file}".', 2)

    try:
        with open(manifest_file, "r", encoding="utf-8") as fh:
            raw = fh.read()
    except OSError as err:
        raise LLMScoutError(f'Could not read fleet manifest "{manifest_file}": {err}', 2) from err

    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError as err:
        raise LLMScoutError(f'Fleet manifest "{manifest_file}" is not valid JSON: {err}', 2) from err
    ...
    for i, entry in enumerate(parsed["sites"]):
        if not isinstance(entry, dict):
            raise LLMScoutError(f'Fleet manifest entry at index {i} must be an object with {{name, path}}.', 2)
        name = entry.get("name")
        raw_path = entry.get("path")
        if not isinstance(name, str) or name.strip() == "":
            raise LLMScoutError(f'Fleet manifest entry at index {i} is missing a "name".', 2)
        if not isinstance(raw_path, str) or raw_path.strip() == "":
            raise LLMScoutError(f'Fleet manifest entry at index {i} is missing a "path".', 2)
