...
def default_config(site_url: str = "") -> LLMScoutConfig:
    return LLMScoutConfig(site_url=site_url, checks={"technical": True, "geo": True})
...
def load_config(project_path: str) -> LLMScoutConfig:
    ...
    try:
        ...
        raise LLMScoutError(f"Could not read {file}: {err}", 2) from err

    try:
        parsed = json.loads(raw)
    except json.JSONDecodeError as err:
        raise LLMScoutError(f"{file} is not valid JSON: {err}", 2) from err

    if not isinstance(parsed, dict):
        raise LLMScoutError(f"{file} must contain a JSON object.", 2)
    ...
    return LLMScoutConfig(site_url=site_url, checks=checks)


def select_checks(config: LLMScoutConfig) -> List[Check]:
