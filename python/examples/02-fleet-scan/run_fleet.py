...
own seofleet.json declaring a real site URL, checked in one pass via
run_fleet(). This is the library equivalent of `seofleet fleet
...
from seofleet import run_fleet
from seofleet.format import format_fleet_results_text
...
def main() -> None:
    scratch = Path(tempfile.mkdtemp(prefix="seofleet-fleet-example-"))
    try:
        manifest_entries = []
        for name, site_url in SITES.items():
            project_dir = scratch / "clients" / name
            project_dir.mkdir(parents=True)
            (project_dir / "seofleet.json").write_text(
