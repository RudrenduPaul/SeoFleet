...
export const CONFIG_FILENAME = "seofleet.json";

export interface SeoFleetConfig {
  /**
   * The live URL that `check`/`fleet` run their checks against. Left blank
   * by `init` -- the user must set it once, since SeoFleet has no way to
...
export function defaultConfig(siteUrl = ""): SeoFleetConfig {
  return {
    siteUrl,
 ...
 */
export function loadConfig(projectPath: string): SeoFleetConfig {
  const file = configPath(projectPath);

  if (!existsSync(file)) {
    throw new SeoFleetError(
      `No ${CONFIG_FILENAME} found in "${projectPath}". Run \`seofleet init ${projectPath}\` first.`,
      2,
    );
  }
  ...
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    throw new SeoFleetError(`${file} must contain a JSON object.`, 2);
  }
  ...
  const checks: SeoFleetConfig["checks"] =
...
export function selectChecks(config: SeoFleetConfig): Check[] {
