...
SeoFleet runs 12 technical-SEO and GEO (generative-engine-optimization) checks against your site from a single Node CLI, with zero Python and zero headless-browser dependency, so a fresh install behaves the same on macOS, Linux, and Windows.
...
SeoFleet is a TypeScript/Node CLI. It targets Node 18 or newer (declared in `package.json` `engines`).
...
SeoFleet is at v0.1 and is not yet published to npm.
