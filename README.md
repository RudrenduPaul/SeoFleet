...
- **Zero external toolchain.** Pure TypeScript/Node. `child_process` is never imported anywhere in the source. The checks run inside the Node process instead of shelling out to Python scripts or a headless browser. This is the entire reason the tool exists (see [below](#what-is-LLMScout-and-why-does-it-exist)).
...
- **Fleet mode.** `LLMScout fleet manifest.json` runs the full suite across many local client-repo paths declared in one JSON manifest, in a single invocation. It reads the local filesystem only, with no SSH and no remote execution.
